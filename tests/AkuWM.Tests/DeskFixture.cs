...
public sealed class DeskFixture
{
    ...
    public DeskFixture(AkuWmConfig config, params MonitorSnapshot[] monitors)
    {
        ...
        Desk.ReadsTheButtonsWith(() => Platform.ButtonsDown);
    ...
    /// <summary>The screens as the platform now lists them, and the burst is over: placements may follow. The longer wait, the one a missing screen gets.</summary>
    public void Screens()
    {
        Desk.SetMonitors(Platform.Monitors());
        Wait(Desk.MonitorReturnMs);
    }
